...
import components.blog_content
import components.local_content
import components.datalab_content
...
def get_logo_image():
    ...
    if logo_image is None:
        logo_image = ui.image('assets/images/logo.gif').style('width: 4rem; height: 4rem;')
    return logo_image
...
def with_auth_layout(route_handler):
    ...
    def wrapper(*args, **kwargs):
        ...
        ui.add_head_html('<link rel="preload" href="/assets/images/logo.gif" as="image">')
        ...
        with ui.column().classes('items-center justify-center min-h-screen w-full'):
            with ui.card().classes('p-8 max-w-md w-full'):
                ...
                with ui.row().classes('w-full justify-center mb-6'):
                    ui.html('<div style="width: 3rem; height: 3rem; background-image: url(\'/assets/images/logo.gif\'); background-size: contain; background-repeat: no-repeat; background-position: center;"></div>')
...
@with_auth_layout
def blog_page():
    components.blog_content.content()

@ui.page('/trends/local')
@with_auth_layout
def local_page():
    components.local_content.content()

@ui.page('/trends/datalab')
@with_auth_layout
def datalab_page():
    components.datalab_content.content()
...
def dashboard():
    ...
    try:
        ...
        with ui.column().classes('w-full max-w-6xl mx-auto p-8'):
            ui.label('Trendis 대시보드').classes('text-4xl font-bold mb-8')
            
            with ui.row().classes('w-full gap-4'):
                # 블로그 검색 카드
                with ui.card().classes('flex-1 hover:shadow-xl cursor-pointer').on('click', lambda: ui.navigate.to('/trends/blog')):
                    with ui.card_section():
                        ui.icon('article', size='xl').classes('text-blue-500')
                        ui.label('블로그 트렌드 검색').classes('text-xl font-bold mt-2')
                        ui.label('네이버 블로그의 최신 트렌드 분석').classes('text-sm text-gray-500')
                
                # 지역 검색 카드
                with ui.card().classes('flex-1 hover:shadow-xl cursor-pointer').on('click', lambda: ui.navigate.to('/trends/local')):
                    with ui.card_section():
                        ui.icon('location_on', size='xl').classes('text-green-500')
                        ui.label('지역 트렌드 검색').classes('text-xl font-bold mt-2')
                        ui.label('지역별 업체 및 장소 분석').classes('text-sm text-gray-500')
                
                # 데이터랩 카드
                with ui.card().classes('flex-1 hover:shadow-xl cursor-pointer').on('click', lambda: ui.navigate.to('/trends/datalab')):
                    with ui.card_section():
                        ui.icon('analytics', size='xl').classes('text-purple-500')
                        ui.label('데이터랩 트렌드').classes('text-xl font-bold mt-2')
                        ui.label('키워드별 검색량 추이 분석').classes('text-sm text-gray-500')
